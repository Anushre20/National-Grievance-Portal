...
export default function CitizenChat() {
  ...
  const { complaints, messages, addMessage, currentUser, officers } = useApp();
  const [newMessage, setNewMessage] = useState("");

  const complaint = complaints.find(c => c.id === complaintId);
  const chatMessages = messages.filter(m => m.complaintId === complaintId);
  const officer = officers.find(o => o.id === complaint?.assignedOfficerId);
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      const response = {
        id: `msg${Date.now() + 1}`,
        complaintId: complaintId!,
        senderId: officer?.id || "o1",
        senderRole: "officer" as const,
        senderName: officer?.name || "Officer",
        message: "Thank you for your message. I'm looking into this matter and will update you shortly.",
        timestamp: new Date(),
      };
      addMessage(response);
    }, 2000);
                  ...
                  {chatMessages.length === 0 ? (
                    ...
                    chatMessages.map((msg) => (
                      <div
                        key={msg.id}
                        className={`flex ${
                          msg.senderRole === "citizen" ? "justify-end" : "justify-start"
                        }`}
                      >
                        <div
                        ...
                        >
                          ...
                          <p
                            className={`text-xs mt-2 ${
                              msg.senderRole === "citizen" ? "text-blue-100" : "text-gray-500"
                            }`}
                          >
